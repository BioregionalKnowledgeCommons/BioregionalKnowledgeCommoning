...
A design choice we're particularly aligned on: BKC separates commitment issuance from pool curation from verification as three orthogonal operations. Anyone can make a promise — self-sovereign issuance, just as in your three-layer model. Pool stewards curate which commitments belong, analogous to SPROUT License §3.5 peer review. Verification is an independent trust signal earned through witnessed follow-through. A commitment can be curated into a pool before it's verified, or verified without being in any pool. And forkability is the safety valve — if a pool's curation diverges from community values, the community takes their history and rebuilds.
