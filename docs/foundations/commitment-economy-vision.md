...
What makes those workshop outputs *matchable* is structure. [RegenCHOICE](https://wiki.simongrant.org/doku.php/ch:index) (Simon Grant) provides a useful model: communities of practice maintain **commons-governed question sets** for their domain, and matching happens on criteria answers rather than free text ([Question system](https://wiki.simongrant.org/doku.php/ch:question_system)). Applied here: each landscape group curates the questions that matter for their bioregion — watershed skills, seasonal availability, equipment access, monitoring methods. Workshop participants answer these questions as part of articulating offers and needs, and the answers feed directly into the [intent publication layer](../ge-integration/intent-publication.md) for bilateral matching. The question library evolves across mapping seasons as landscape groups learn what criteria actually predict successful commitment fulfillment.
