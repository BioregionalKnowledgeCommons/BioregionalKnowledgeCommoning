...
|----------|------------------------|---------|-------|
| Embedding model | Stay with text-embedding-3-small, evaluate after contextual retrieval. | **Switched to Qwen3-0.6B** | Qwen/Qwen3-Embedding-0.6B (1024-dim) via centralized FastAPI service on poly (10.100.0.1:8352). Octo + personal KOI both use RemoteEmbeddingProvider. Eval gate: CR=0.435 (-3.8% vs OpenAI, within 5% threshold). Instruction prefix required (`prompt_name="query"` for retrieval — without it, -21.7% regression). Bakeoff: bge-large failed (-9.8%), Qwen3 without prefix failed (-21.7%), Qwen3 with prefix passed (-3.8%). Monthly cost: $0. Latency: p95=321ms (was 3.6s on CPU). |
...
| 2026-04-02 | Sprint 3 closure | golden-qa-100 complete (52→100 questions, frozen baseline CR=0.452/gpt-4.1-mini). Embedding switch: 3-model bakeoff (bge-large -9.8%, Qwen3 without prefix -21.7%, Qwen3 with prefix -3.8% PASS). Poly embedding service deployed (p95=321ms, was 3.6s). Personal KOI migrated to 1024-dim. Key lessons: MTEB ≠ domain performance, instruction prefix is critical, H200 re-embed 200x faster than CPU. Eval preflight + RUNTIME_VECTOR_COLUMNS startup guard added. |
